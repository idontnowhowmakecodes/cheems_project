...
class GestureSessionReport:
    ...
    def _draw_summary(
        ...
        """Muestra los datos de contexto y conteos principales de la sesión."""
        ...
        text = "\n".join(
            (
                ...
                f"Observaciones guardadas: {len(observations)}",
                f"Marcas del terapeuta: {sum(event.event_type == 'attempt_marked' for event in events)}",
            )
        )
        axis.axis("off")
    ...
    def _draw_timeline(
        ...
        """Muestra detecciones de mano y marcas manuales a lo largo del tiempo."""
        start_event = next(
        ...
        for event in events:
            if event.event_type == "attempt_marked":
                axis.axvline(
                    (event.timestamp_ms - start_timestamp_ms) / 1000,
                    color="#e76f51",
                    linestyle="--",
